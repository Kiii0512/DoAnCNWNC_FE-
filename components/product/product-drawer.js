import { createProduct, updateProductFull } from '../../JS/API/adminproductAPI.js';
...
class ProductDrawer extends HTMLElement {
  ...
  const [cats, brands] = await Promise.all([
    getCategories(), // lấy tất cả, filter FE
    getBrands()      // lấy tất cả, filter FE
  ]);

  /* ===== CATEGORY: chỉ lấy active ===== */
  const activeCategories = cats.filter(c => c.isActive === true);
  ...
  let brandOptions = brands.filter(b => b.isActive === true);

  /**
   * 👉 FIX QUAN TRỌNG CHO EDIT MODE
   * Nếu đang edit và brand hiện tại bị inactive
   * → vẫn phải add nó vào combobox
   */
  if (editMode && editingProductId) {
    const currentBrandId = state.original?.product?.brandId;
    const currentBrand = brands.find(b => b.brandId === currentBrandId);

    if (currentBrand && !currentBrand.isActive) {
      brandOptions = [currentBrand, ...brandOptions];
    }
  }
  ...
  this.brand.innerHTML = `
    ...
    ${brandOptions.map(b =>
      `<option value="${b.brandId}">
        ${b.brandName}${b.isActive ? '' : ' (Ngừng hoạt động)'}
      </option>`
    ).join('')}
  `;
}
