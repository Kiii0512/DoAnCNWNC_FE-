...
// header buttons behavior chắc chắn chuyển trang giỏ hàng
document.addEventListener('DOMContentLoaded', function () {
  const btnGioElem = document.getElementById('btnGio') || document.getElementById('cartBtn');

  if (btnGioElem) {
    // xoá hết listener cũ (nếu có) bằng cách thay thế element
    try {
      btnGioElem.replaceWith(btnGioElem.cloneNode(true));
   
      const newBtn = document.getElementById('btnGio') || document.getElementById('cartBtn');
      newBtn.addEventListener('click', function (e) {
        // nếu là thẻ <a> có href thì giữ nguyên hành vi mặc định
        if (newBtn.tagName && newBtn.tagName.toLowerCase() === 'a' && newBtn.getAttribute('href')) {
          return;
        }

        // còn nếu không thì chuyển trang giỏ hàng
        // nếu có logic khác (như mở sidebar giỏ hàng) thì thay thế đoạn này
        window.location.href = 'gioHang.html';
      });
    } catch (err) {
      // đơn giản thêm listener mới nếu không thể thay thế element
      btnGioElem.addEventListener('click', function (e) {
        if (btnGioElem.tagName && btnGioElem.tagName.toLowerCase() === 'a' && btnGioElem.getAttribute('href')) {
          return;
        }
        window.location.href = 'gioHang.html';
      });
...
// phải sửa lại hành vi nút tài khoản như button giỏ hàng cho sau này
  // demo cho nút tài khoản
  const btnTK = document.getElementById('btnTK') || document.getElementById('taikhoan');
  if (btnTK) {
    btnTK.addEventListener('click', () => { alert('Chức năng Tài khoản (demo)'); });
  }
});
